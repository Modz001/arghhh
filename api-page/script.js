...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const populatePageContent = () => {
        ...
        const creator = settings.apiSettings?.creator || 'Modz Never Die';

        setPageContent(DOM.pageTitle, settings.name, "Modz API");
        setPageContent(DOM.wm, `© ${currentYear} ${creator}. Semua Hak Dilindungi.`);
        setPageContent(DOM.appName, settings.name, "Modz API");
        setPageContent(DOM.sideNavName, settings.name || "API");
        setPageContent(DOM.versionBadge, settings.version, "V17.1.11");
        ...
        if (DOM.apiLinksContainer) {
            ...
            const defaultLinks = [{ url: "https://modzweb.biz.id", name: "Modz Web", icon: "fas fa-globe" }];
    ...
    const displayErrorState = (message) => {
        ...
        DOM.apiContent.innerHTML = `
            <div class="no-results-message text-center p-5">
                ...
                <button class="btn btn-primary mt-3" onclick="location.reload()">Muat Ulang
                </button>
            </div>
        `;
    };
    ...
    const createNoResultsMessage = () => {
        ...
        if (!noResultsMsg) {
            ...
            noResultsMsg.innerHTML = `
                ...
                <button id="clearSearchFromMsg" class="btn btn-primary mt-3">Hapus Pencarian
                </button>
            `;
